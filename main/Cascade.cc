...
PYBIND11_MODULE(_cascade, m)
{
    py::class_<AMCM>(m, "AMCM")
        .def(py::init<>())
        .def("register_module",
             py::overload_cast<const std::string &>(&AMCM::RegisterModule),
        ...
        .def("run_module",
             py::overload_cast<const std::string &>(&AMCM::RunAModule))
        .def("run_module", py::overload_cast<std::shared_ptr<IAnalysisModule>>(
                               &AMCM::RunAModule))
        .def("sequential_run", &AMCM::SequentialRun)
        .def("run_group", py::overload_cast<const std::vector<std::string> &>(
                              &AMCM::RunModules))
        .def("run_group",
             py::overload_cast<std::vector<std::shared_ptr<IAnalysisModule>>>(
                 &AMCM::RunModules))
    ...
    m.def("set_log_level", [](logger::LogLevel level)
          { logger::Logger::Get().SetLogLevel(level); });
    m.def("set_log_file", [](const std::string &path)
          { logger::Logger::Get().InitLogFile(path); });
    ...
    py::class_<IAnalysisModule, std::shared_ptr<IAnalysisModule>>(
        m, "IAnalysisModule")
