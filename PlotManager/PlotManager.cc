...
vector<string> PlotManager::GetPrefixList() { return vector<string>(samples.begin(), samples.end()); }
    ...
    for (auto &file : fileList)
    {
        ...
        while ((key = static_cast<TKey *>(next())))
        {
            ...
            if (obj->InheritsFrom(TH1::Class()))
            {
                ...
                TString prefix_t = parsename(parsename.Last('_') + 1, parsename.Length());
                const char *alias = alias_t.Data();
                const char *prefix = prefix_t.Data();
                TH1 *hist = static_cast<TH1 *>(obj);
                hist->SetDirectory(nullptr);
                auto cloned = std::unique_ptr<TH1>(static_cast<TH1 *>(hist->Clone()));
    ...
    for (auto &prefix : prefixes)
    {
        for (auto &[alias, hists] : histMap)
        {
            for (auto &hist : hists)
            {
                ...
                if (hname.EndsWith(prefix.c_str())) hstacks[alias]->Add(hist.get());
            }
        }
    }
    return hstacks;
...
map<string, TH1 *> PlotManager::ExtractPrefix(const string &prefix)
{
    ...
    for (auto &[alias, hists] : histMap)
    {
        ...
        {
            TString hname = hist->GetName();
            if (hname.EndsWith(prefix.c_str())) hmap[alias] = hist.get();
        }
    }
    return hmap;
}
...
void PlotManager::StyleHists(const vector<string> &prefixes, const vector<int> &fillcolor, const vector<int> &linecolor)
{
    for (auto &[alias, hists] : histMap)
    {
        for (auto &hist : hists)
...
void PlotManager::MergeHists(const string &newprefix, const vector<string> &prefixes)
{
    samples.insert(newprefix);
    for (auto &[alias, hists] : histMap)
    {
        auto hmerged = unique_ptr<TH1>(static_cast<TH1 *>(histMap[alias][0]->Clone(Form("hist_%s_%s", alias.c_str(), newprefix.c_str()))));
        hmerged->Reset();
...
void PlotManager::SetLuminosity(double lum = 0)
{
    ...
    luminosity = Form("#scale[0.85]{#bf{#int#it{L}dt = %s %s}}", temp.Data(), invban.Data());
    isLumCalled = true;
}
...
double PlotManager::SetCanvasAuto(int input)
{
    ...
    if (mcstat)
    {
        ...
        for (int i = 0; i < gr.size(); i++)
        {
            ...
            {
                double VAL = multf * (gr[i]->GetPointY(j) + abs(gr[i]->GetErrorYhigh(j)));
                if (vperr < VAL) vperr = VAL;
            }
        }
        for (int i = 0; i < h.size(); i++)
        {
            double VAL = multf * (h[i]->GetMaximum() + h[i]->GetBinErrorUp(h[i]->GetMaximumBin()));
            if (vperr < VAL) vperr = VAL;
        }
...
void PlotManager::PullPadSet(TPad *p1, TPad *p2, TH1 *pullFrame)
{
    ...
    pullFrame->GetYaxis()->SetLabelFont(43); // Absolute font size in pixel (precision 3)
    ...
    pullFrame->GetXaxis()->SetLabelFont(43); // Absolute font size in pixel (precision 3)
...
void PlotManager::SetLegendAuto(const vector<int> &colorscheme, const vector<TString> &components)
{
    ...
    for (int i = 0; i < entries; i++)
        dummy.push_back(make_unique<TH1F>(Form("hdummy_%d__legend", i), "", 1, 0, 1));

    for (int i = 0; i < entries; i++)
    {
        if (components[i].Contains("sig", TString::kIgnoreCase) || components[i].Contains("K#eta#nu", TString::kIgnoreCase))
        {
...
double PlotManager::SetSignalScale(TH1 *hsignal, THStack *hs, double scale = 1.0)
{
    TH1 *hbkg = (TH1 *)(hs->GetStack()->Last()->Clone(Form("hbkg_%ld", time(NULL) % 100000)));
...
void PlotManager::DrawLatex()
{
    ...
    if (isExpCalled)
    {
        ...
        latex->DrawLatexNDC(0.2, 0.87, Form("#it{%s} #bf{%s}", experiment.Data(), comment.Data()));
    }
    if (isLumCalled)
    {
    ...
    if (asymmerr)
    {
        for (int i = 0; i < h->GetNbinsX(); i++)
        {
            ...
            else
            {
                ...
                elow = (sw / n_eff) * (n_eff - ROOT::Math::gamma_quantile(alpha / 2, n_eff, 1.0));
                eup = (sw / n_eff) * (ROOT::Math::gamma_quantile_c(alpha / 2., n_eff + 1, 1.0) - n_eff);
                if (norm)
                {
                    gr->AddPoint(h->GetBinCenter(i + 1), 1.0);
                    if (errbar)
                        gr->SetPointError(i, bwidth / 2., bwidth / 2., elow / sw, eup / sw);
                    else
                ...
                else
                {
                    ...
                    if (errbar)
                        gr->SetPointError(i, bwidth / 2., bwidth / 2., elow, eup);
                    else
    ...
    else
    {
        for (int i = 0; i < h->GetNbinsX(); i++)
        {
            if (norm) // for lower frame plot
            {
                ...
                if (errbar)
                    gr->SetPointError(i, h->GetBinWidth(i + 1) / 2., h->GetBinWidth(i + 1) / 2., h->GetBinErrorLow(i + 1) / h->GetBinContent(i + 1),
                                      h->GetBinErrorUp(i + 1) / h->GetBinContent(i + 1));
                else
                    gr->SetPointError(i, 0, 0, h->GetBinErrorLow(i + 1) / h->GetBinContent(i + 1), h->GetBinErrorUp(i + 1) / h->GetBinContent(i + 1));
            }
            else // mc stat
            {
                gr->AddPoint(h->GetBinCenter(i + 1), h->GetBinContent(i + 1));
                if (errbar)
                    gr->SetPointError(i, h->GetBinWidth(i + 1) / 2., h->GetBinWidth(i + 1) / 2., h->GetBinErrorLow(i + 1), h->GetBinErrorUp(i + 1));
                else
                    gr->SetPointError(i, 0, 0, h->GetBinErrorLow(i + 1), h->GetBinErrorUp(i + 1));
            }
        }
    }
    gr->SetFillColor(1);
...
TGraphAsymmErrors *PlotManager::HistToGraph(THStack *hs, int input)
{
    ...
    TH1 *h = (TH1 *)(hs->GetStack()->Last()->Clone(Form("mch_%s", hs->GetName())));
...
TObjArray *PlotManager::MakeRatioPlot(THStack *hmc, TH1 *hdata, int input)
{
    ...
    TH1 *hmctot = (TH1 *)(hmc->GetStack()->Last()->Clone(Form("mctotof_%s", hdata->GetName())));
    ...
    else
    {
        ...
        if (mcerrbar)
        {
            mcerrgraph = HistToGraph(hmctot, PM::Draw::kMCerr | asymerrbit | PM::Draw::kNorm);
    ...
    if (arrow)
    {
        ...
        {
            if (ratiographerr->GetPointY(i) > 2) vecArrow.push_back(ratiographerr->GetPointX(i));
        }
        TObjArray *arrowArr = new TObjArray(vecArrow.size());

        for (int i = 0; i < vecArrow.size(); i++)
            arrowArr->Add(new TArrow(vecArrow[i], 1.95, vecArrow[i], 2, 0.015, "|>"));
...
void PlotManager::PlotSingle(const string &prefix, const string &var, const string &title, int opts)
{
    TH1 *h = nullptr;
    ...
    if (it != histMap.end())
    {
        ...
        {
            TString hname = hist->GetName();
            if (hname.EndsWith(prefix.c_str())) h = hist.get();
        }
    }
    else
    {
    ...
    c1->cd(1);
    h->SetTitle(Form(";%s;Candidates per %g %s", (title.length() == 0) ? var.c_str() : title.c_str(), h->GetBinWidth(1), ExtractUnit(title).c_str()));
    h->Draw("HIST");
    if (opts & PM::Draw::kMCerr) errgr->Draw("E2,SAME");
    SetCanvasAuto(opts);
    if (opts & PM::Draw::kLatex) DrawLatex();
    if (opts & PM::Draw::kLegend) DrawLegend();
    c1->Update();
    canvasList.push_back({var, c1});
}

void PlotManager::PlotMultiple(const vector<vector<string>> &prefixes, const string &var, const string &title, int opts)
{
    ...
    for (auto &all : prefixes)
    {
        if (all.size() == 1)
        {
            ...
            if (it != allhists.end())
            {
                ...
                if (binwidth < 0) binwidth = it->second->GetBinWidth(1);
            }
        }
        else
        {
            ...
            {
                hists.push_back(it->second);
                if (binwidth < 0) binwidth = static_cast<TH1 *>(it->second->GetStack()->Last())->GetBinWidth(1);
            }
        }
    }
    ...
    for (auto &h : hists)
    {
        if (first)
        {
            (h->InheritsFrom("TH1")) ? static_cast<TH1 *>(h)->SetTitle(Form(";%s;Candidates per %g %s", (title.length() == 0) ? var.c_str() : title.c_str(),
                                                                            binwidth, ExtractUnit(title).c_str()))
                                     : static_cast<THStack *>(h)->SetTitle(Form(";%s;Candidates per %g %s", (title.length() == 0) ? var.c_str() : title.c_str(),
                                                                                binwidth, ExtractUnit(title).c_str()));
    ...
    if (opts & PM::Draw::kSigEmb) hsignal->Draw("HIST,SAME");
    if (opts & PM::Draw::kLatex) DrawLatex();
    if (opts & PM::Draw::kLegend) DrawLegend();
    canvasList.push_back({var, c1});
}
void PlotManager::PlotComparison(const string &var, const vector<string> &prefixes_num, const string &prefix_den, const string &title, int opts)
{
    ...
    TCanvas *c1 = new TCanvas(Form("c_%s_%d_compare", var.c_str(), opts), "", 800, 700);
    if (opts & PM::Draw::kRatio)
    {
        ...
        p1->cd();
        hbkg->SetTitle(Form(";;Candidates per %g %s", hdata->GetBinWidth(1), ExtractUnit(title).c_str()));
        hbkg->Draw("HIST");
        if (opts & PM::Draw::kSigEmb) hsignal->Draw("HIST,SAME");
        hdata->Draw("PE,SAME");
        if (opts & PM::Draw::kMCerr) mcerrgr->Draw("E2,SAME");
        SetCanvasAuto(opts);
        if (opts & PM::Draw::kKtest)
        {
            ktestprob = ((TH1 *)(hbkg->GetStack()->Last()))->KolmogorovTest(hdata);
            ktest->SetTextSize(0.03);
            ktest->SetTextAlign(13);
            ktest->DrawLatexNDC(0.2, 0.76, Form("Kolmogorov Test p-Value = %.3lf", ktestprob));
        }
        if (opts & PM::Draw::kLatex) DrawLatex();
        if (opts & PM::Draw::kLegend) DrawLegend();
        ...
        static_cast<TH1 *>((*arr)[0])->SetTitle(Form(";%s;Data/MC", (title.length() == 0) ? var.c_str() : title.c_str()));
        (*arr)[0]->Draw("AXIS");
        (*arr)[1]->Draw("SAME,PE");
        if (opts & PM::Draw::kMCerr) (*arr)[2]->Draw("SAME,E2");
    ...
    else
    {
        ...
        c1->cd(1);
        hbkg->SetTitle(
            Form(";%s;Candidates per %g %s", (title.length() == 0) ? var.c_str() : title.c_str(), hdata->GetBinWidth(1), ExtractUnit(title).c_str()));
        hbkg->Draw("HIST");
        if (opts & PM::Draw::kSigEmb) hsignal->Draw("HIST,SAME");
        hdata->Draw("PE,SAME");
        if (opts & PM::Draw::kMCerr) mcerrgr->Draw("SAME,E2");
        SetCanvasAuto(opts);
        if (opts & PM::Draw::kKtest)
        {
            ktestprob = ((TH1 *)(hbkg->GetStack()->Last()))->KolmogorovTest(hdata);
...
void PlotManager::RegisterCanvas(const string &name, TCanvas *c) { canvasList.push_back({name, c}); }
...
void PlotManager::SaveDrawnCanvases(const string &suffix, const string &comment)
{
    ...
    {
        if (comment.length() == 0)
            can.second->Print(Form("%s/%s.%s", outputDir.c_str(), can.first.c_str(), suffix.c_str()));
        else
            can.second->Print(Form("%s/%s_%s.%s", outputDir.c_str(), can.first.c_str(), comment.c_str(), suffix.c_str()));
    }
}
