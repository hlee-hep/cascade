...
std::string Logger::colored(const std::string &text, const std::string &color,
...
std::string
Logger::ReplaceRegex(const std::string &input, const std::regex &pattern,
...
std::string Logger::HighlightStatus(std::string msg)
{
    const std::vector<
        ...
        status_map = {{"Status : Initializing", {"magenta", "bold"}},
    ...
    for (const auto &[key, style] : status_map)
    {
        ...
        if (pos != std::string::npos)
            msg.replace(pos, key.size(),
                        colored(key, style.first, style.second));
    }
    return msg;
}
...
std::string Logger::HighlightFileNames(std::string msg)
{
    ...
    return ReplaceRegex(msg, file_pattern, [&](const std::smatch &m)
                        { return colored(m.str(), "green", "bold"); });
}

std::string Logger::HighlightExprs(std::string msg)
{
    msg = ReplaceRegex(msg, std::regex(R"((name :\s*)(\w+))"),
                       [&](const std::smatch &m)
                       { return m[1].str() + colored(m[2].str(), "cyan"); });
    msg = ReplaceRegex(msg, std::regex(R"((expr :\s*)([^,;\n]+))"),
                       [&](const std::smatch &m)
                       { return m[1].str() + colored(m[2].str(), "yellow"); });
    return msg;
}
...
std::string Logger::HighlightParams(std::string msg)
{
    ...
    return ReplaceRegex(msg, kv_pattern,
                        [&](const std::smatch &m)
                        {
                            return colored(m[1].str(), "magenta", "bold") +
                                   " = " + colored(m[2].str(), "white");
                        });
}
...
std::string Logger::HighlightWarnings(std::string msg)
{
    const std::vector<std::string> keywords = {
        "Duplication of hash is detected",
        "Invalid bin format",
        "Missing snapshot",
        "Failed to open",
        "Continue",
    ...
    for (const auto &k : keywords)
    {
        auto pos = msg.find(k);
        if (pos != std::string::npos)
            msg.replace(pos, k.size(), colored(k, "red", "bold"));
    }
    return msg;
}
...
std::string Logger::ApplyColor(LogLevel level, const std::string &module,
                               const std::string &msg)
{
    std::string level_str =
        colored("[" + ToString(level) + "]", LevelToColor(level));
    std::string mod_str;
                              ...
                              double elapsed, double eta)
{
    ...
    oss << std::fixed << colored("[INFO]", "cyan") << " ["
        << colored("LOOP", "red", "bold") << "] [";
    ...
    if (eta >= 0)
        oss << " | ETA: " << std::fixed << std::setprecision(1) << eta << "s";
